...
│   │   └── ...
│   └── outputs/
│       └── valid_single_day_basic.csv   # example CLI export, regenerated by `python -m app.main`
├── tests/
│   ├── execution/
│   ├── productivity/
│   ├── ui/
...
> `tests/` contains an automated suite (`python -m pytest`) covering the optimizer's core modules (constraints, PERT/dependencies, reward scoring, the optimizer itself, CSV loading, and CLI time formatting), execution tracking, productivity analytics, and the UI-facing controllers.
...
The reward system can read values from a YAML file, but only when a `config_path` is passed explicitly to `load_reward_settings()`/`optimize_day_schedule()`. **Neither the CLI (`app/main.py`) nor the desktop UI (`app/app.py`) currently passes one**, and the file's built-in default search does not match this repository's actual `config/task_preferences.yaml` (it looks for singular/misspelled variants like `task_prefrence.yaml`/`task_preference.yaml`, not the plural, correctly-spelled filename that is actually checked in). In practice, today, scheduling always runs on `RewardSettings`' built-in defaults, not on `config/task_preferences.yaml`'s values. If the YAML file is missing, incomplete, or (as today) never discovered, safe default values are used regardless.
...
This is designed to let the optimizer's behavior be tuned without rewriting Python code, **but see the reward-config discovery limitation above and in [Current Limitations](#current-limitations)**: today, this file is only actually used if its path is passed explicitly as `config_path`; neither the CLI nor the desktop UI does so by default, so editing it currently has no effect on `python -m app.main` or `python -m app.app`.
...
- **Reward-config file discovery does not match the checked-in filename.** `config/task_preferences.yaml` (plural) exists, but `app/reward.py`'s default (no explicit `config_path`) search only looks for `task_prefrence.yaml`/`task_prefrence.yml`/`task_preference.yaml`/`task_preference.yml`. Neither `app/main.py` nor `app/app.py` passes an explicit `config_path`, so today `config/task_preferences.yaml` has no effect on scheduling and the optimizer always runs on `RewardSettings`' built-in defaults. Pass `config_path` explicitly to `load_reward_settings()`/`optimize_day_schedule()` to actually use a YAML file.
